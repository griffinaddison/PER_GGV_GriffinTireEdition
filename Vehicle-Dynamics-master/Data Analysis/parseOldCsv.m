function [IDs, messages] = parseOldCsv(filename)
    ...
    clearvars raw_data
    raw_data = textscan(fileID, '%s %s %u', 1000, 'HeaderLines',1, 'Delimiter', {' '});
    ...
    raw_values = textscan(fileID, '%f %f %f', 'HeaderLines',1+length(IDs), 'Delimiter', {' ', ','});

    k = keys(IDs);
    v = cell2mat(values(IDs));
    mergedValues = [raw_values{1}, raw_values{2}, raw_values{3}];

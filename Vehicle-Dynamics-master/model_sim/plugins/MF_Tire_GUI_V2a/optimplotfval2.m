...
if iteration == 0
    ...
    title('MATLAB:funfun:optimplots:TitleCurrentFunctionValue','interp','none');
    %xlabel(getString(message('MATLAB:funfun:optimplots:LabelIteration')),'interp','none');
    xlabel('MATLAB:funfun:optimplots:LabelIteration','interp','none');
    set(plotfval,'Tag','optimplotfval');
    %ylabel(getString(message('MATLAB:funfun:optimplots:LabelFunctionValue')),'interp','none')
    ylabel('MATLAB:funfun:optimplots:LabelFunctionValue','interp','none')
else
    ...
    set(get(gca,'Title'),'String','MATLAB:funfun:optimplots:TitleCurrentFunctionValue');
end
...
if iteration == 0
    ...
    xlabelText = 'MATLAB:funfun:optimplots:LabelNumberOfFunctionValues0';
    ...
    if numel(fval) > 100
        ...
        xlabelText = {xlabelText,'MATLAB:funfun:optimplots:LabelShowingOnlyFirst100Values'};
        fval = fval(1:100);
    end
    plotfval = bar(fval);
    %title(getString(message('MATLAB:funfun:optimplots:TitleCurrentFunctionValues')),'interp','none');
    title('MATLAB:funfun:optimplots:TitleCurrentFunctionValues','interp','none');
